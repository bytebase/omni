...
type pendingATCmd struct {
	...
	identityOptions *nodes.List
}

func alterTableRecurse(rv *nodes.RangeVar) bool {
	if rv == nil {
		return true
	}
	if rv.Inh {
	...
	// Hand-built tests often omit RangeVar.Inh and Loc, while parser-built
	// ALTER TABLE ONLY has a real source location and Inh=false.
	return rv.Loc.Start == 0 && rv.Loc.End == 0
}

func (c *Catalog) transformAlterTableCmd(atc *nodes.AlterTableCmd) (*nodes.AlterTableCmd, *nodes.List, error) {
	if nodes.AlterTableType(atc.Subtype) != nodes.AT_AddIdentity {
		return atc, nil, nil
	}

	switch def := atc.Def.(type) {
	case *nodes.Constraint:
		if def.Contype != nodes.CONSTR_IDENTITY {
			return nil, nil, fmt.Errorf("AT_AddIdentity: expected identity Constraint")
		}
		transformed := *atc
		transformed.Def = &nodes.ColumnDef{
			Colname:  atc.Name,
			Identity: def.GeneratedWhen,
			Loc:      nodes.NoLoc(),
		}
		return &transformed, def.Options, nil
	case *nodes.ColumnDef:
		return atc, nil, nil
	default:
		return nil, nil, fmt.Errorf("AT_AddIdentity: expected Constraint or ColumnDef")
	}
}

func splitSetIdentityOptions(def nodes.Node) (byte, bool, *nodes.List, error) {
	if def == nil {
		return 0, false, nil, nil
	}
	list, ok := def.(*nodes.List)
	if !ok {
		return 0, false, nil, fmt.Errorf("AT_SetIdentity: expected List")
	}

	var generated byte
	var sawGenerated bool
	var seqOptions []nodes.Node
	for _, item := range list.Items {
		d, ok := item.(*nodes.DefElem)
		if !ok {
			seqOptions = append(seqOptions, item)
			continue
		}
		...
		if sawGenerated {
			return 0, false, nil, &Error{Code: CodeSyntaxError, Message: "conflicting or redundant options"}
		}
		v, ok := defElemInt(d)
		if !ok {
			return 0, false, nil, &Error{Code: CodeSyntaxError, Message: "invalid generated option"}
		}
		generated = byte(v)
		sawGenerated = true
	}

	var seqList *nodes.List
	if len(seqOptions) > 0 {
		seqList = &nodes.List{Items: seqOptions}
	}
	return generated, sawGenerated, seqList, nil
}
...
func (c *Catalog) AlterTableStmt(stmt *nodes.AlterTableStmt) error {
	...
	recurse := alterTableRecurse(stmt.Relation)
	...
	for i, item := range stmt.Cmds.Items {
		...
		transformed, identityOptions, err := c.transformAlterTableCmd(atc)
		if err != nil {
			return err
		}
		pending = append(pending, pendingATCmd{
			pass:            atCmdPass(nodes.AlterTableType(transformed.Subtype)),
			origIdx:         i,
			cmd:             transformed,
	...
	for _, p := range pending {
		if err := c.execAlterTableCmd(schema, rel, relName, p.cmd, p.identityOptions, recurse, false); err != nil {
			return err
		}
		if recurse && shouldRecurse(nodes.AlterTableType(p.cmd.Subtype)) {
			for _, childOID := range c.findChildRelations(rel.OID) {
				childRel := c.relationByOID[childOID]
				if childRel == nil {
					continue
				}
				if err := c.execAlterTableCmd(childRel.Schema, childRel, childRel.Name, p.cmd, p.identityOptions, recurse, true); err != nil {
					return err
				}
			}
		}
...
func (c *Catalog) execAlterTableCmd(schema *Schema, rel *Relation, relName string, atc *nodes.AlterTableCmd, identityOptions *nodes.List, recurse, recursing bool) error {
	...
	case nodes.AT_AddIdentity:
		...
		return c.atAddIdentity(schema, rel, relName, atc.Name, cd, identityOptions, recurse, recursing)

	case nodes.AT_DropIdentity:
		return c.atDropIdentity(rel, atc.Name, atc.Missing_ok, recurse, recursing)

	case nodes.AT_SetIdentity:
		return c.atSetIdentity(rel, atc.Name, atc.Def, recurse, recursing)
	...
	case nodes.AT_AddIndexConstraint:
		...
		if idxStmt.IndexOid != 0 {
			idx := c.indexes[uint32(idxStmt.IndexOid)]
			if idx == nil {
				return errUndefinedObject("index", fmt.Sprintf("%d", idxStmt.IndexOid))
			}
...
func (c *Catalog) atAddColumn(schema *Schema, rel *Relation, relName string, colDef ColumnDef, recursing bool) error {
	...
	var identitySeq *Sequence
	var identityDefault string
	if colDef.Identity == 'a' || colDef.Identity == 'd' {
		if !recursing {
			seq, seqRV, explicitName, err := c.createIdentitySequence(schema, relName, colDef.Name, typeOID, colDef.IdentityOptions)
			if err != nil {
				return err
			}
			colDef.IdentitySequence = seqRV
			identitySeq = seq
			identityDefault = identitySequenceDefault(schema, seq.Schema, seq.Name, explicitName)
		}
		colDef.NotNull = true
	}
	...
	if colDef.Identity == 'a' || colDef.Identity == 'd' {
		col.Identity = colDef.Identity
		if !recursing {
			col.HasDefault = true
			col.Default = identityDefault
		}
	}
	...
	if identitySeq != nil {
		identitySeq.OwnerRelOID = rel.OID
		identitySeq.OwnerAttNum = col.AttNum
		c.recordDependency('s', identitySeq.OID, 0, 'r', rel.OID, int32(col.AttNum), DepAuto)
	}
	return nil
}
...
func (c *Catalog) atAddIdentity(schema *Schema, rel *Relation, relName, colName string, cd *nodes.ColumnDef, identityOptions *nodes.List, recurse, recursing bool) error {
	isPartitioned := rel.RelKind == 'p'
	if isPartitioned && !recurse {
		return &Error{Code: CodeInvalidTableDefinition,
			Message: "cannot add identity to a column of only the partitioned table"}
	}
	if rel.PartitionOf != 0 && !recursing {
		return &Error{Code: CodeInvalidTableDefinition,
	...
	if !col.NotNull {
		return &Error{Code: CodeObjectNotInPrerequisiteState,
			Message: fmt.Sprintf("column %q of relation %q must be declared NOT NULL before identity can be added", colName, rel.Name)}
	}

	if col.Identity != 0 {
		return &Error{Code: CodeObjectNotInPrerequisiteState,
			Message: fmt.Sprintf("column %q of relation %q is already an identity column", colName, rel.Name)}
	}

	if col.HasDefault {
		return &Error{Code: CodeObjectNotInPrerequisiteState,
			Message: fmt.Sprintf("column %q of relation %q already has a default value", colName, rel.Name)}
	}
	...
	if !recursing {
		seq, seqRV, explicitName, err := c.createIdentitySequence(schema, relName, colName, col.TypeOID, identityOptions)
		if err != nil {
			return err
		}
		...
		col.HasDefault = true
		col.Default = identitySequenceDefault(schema, seq.Schema, seq.Name, explicitName)
	}

	col.Identity = identity

	if recurse && isPartitioned {
		for _, childOID := range c.findPartitionChildRelations(rel.OID) {
			childRel := c.relationByOID[childOID]
			if childRel == nil {
				continue
			}
			if err := c.atAddIdentity(childRel.Schema, childRel, childRel.Name, colName, cd, identityOptions, recurse, true); err != nil {
				return err
			}
		}
	}
...
// atSetIdentity changes identity generation mode and/or backing sequence options.
//
// pg: src/backend/commands/tablecmds.c — ATExecSetIdentity
func (c *Catalog) atSetIdentity(rel *Relation, colName string, def nodes.Node, recurse, recursing bool) error {
	isPartitioned := rel.RelKind == 'p'
	if isPartitioned && !recurse {
		return &Error{Code: CodeInvalidTableDefinition,
	...
	generated, sawGenerated, seqOptions, err := splitSetIdentityOptions(def)
	if err != nil {
		return err
	}
	...
	if col.Identity == 0 {
		return &Error{Code: CodeObjectNotInPrerequisiteState,
			Message: fmt.Sprintf("column %q of relation %q is not an identity column", colName, rel.Name)}
	}

	if seqOptions != nil && !recursing {
		seq, err := c.findIdentitySequence(rel, col.AttNum)
		if err != nil {
			return err
		}
		if err := c.AlterSequenceStmt(&nodes.AlterSeqStmt{
			Sequence:    &nodes.RangeVar{Schemaname: seq.Schema.Name, Relname: seq.Name},
			Options:     seqOptions,
			ForIdentity: true,
		}); err != nil {
			return err
	...
	if sawGenerated {
		col.Identity = generated
	}

	if sawGenerated && recurse && isPartitioned {
		for _, childOID := range c.findPartitionChildRelations(rel.OID) {
			childRel := c.relationByOID[childOID]
			if childRel == nil {
				continue
			}
			if err := c.atSetIdentity(childRel, colName, def, recurse, true); err != nil {
				return err
			}
		}
	}
...
func (c *Catalog) atDropIdentity(rel *Relation, colName string, ifExists, recurse, recursing bool) error {
	isPartitioned := rel.RelKind == 'p'
	if isPartitioned && !recurse {
		return &Error{Code: CodeInvalidTableDefinition,
	...
	if recurse && isPartitioned {
		for _, childOID := range c.findPartitionChildRelations(rel.OID) {
			...
			if err := c.atDropIdentity(childRel, colName, false, recurse, true); err != nil {
				return err
			}
		}
	}
...
func (c *Catalog) findPartitionChildRelations(parentOID uint32) []uint32 {
	var children []uint32
	for _, e := range c.inhEntries {
		if e.InhParent != parentOID {
			continue
		}
		child := c.relationByOID[e.InhRelID]
		if child != nil && child.PartitionOf == parentOID {
			children = append(children, e.InhRelID)
		}
	}
	return children
