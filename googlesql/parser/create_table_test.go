...
func TestCreateTable_GeneratedIdentity(t *testing.T) {
	// BigQuery `GENERATED ALWAYS AS IDENTITY(…)` / `GENERATED BY DEFAULT AS
	// IDENTITY(…)` — the generated_mode identity_column_info alternative. IDENTITY
	// is a keyword (kwIDENTITY); the body is recognized and skipped (no generated
	// expression). Rejected by the Spanner emulator (non-authoritative; .g4 +
	// BigQuery docs). Regression for parser-ddl PR #220 finding 4 (the old branch
	// keyed on tokIdentifier, which never matches the IDENTITY keyword → the form
	// was wrongly rejected).
	cases := []struct {
		sql     string
		genMode ast.GeneratedMode
	}{
		{"CREATE TABLE T (id INT64 GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1)) PRIMARY KEY (id)", ast.GenModeGeneratedAlways},
		{"CREATE TABLE T (id INT64 GENERATED BY DEFAULT AS IDENTITY (NO CYCLE)) PRIMARY KEY (id)", ast.GenModeGeneratedByDefault},
		{"CREATE TABLE T (id INT64 GENERATED ALWAYS AS IDENTITY ()) PRIMARY KEY (id)", ast.GenModeGeneratedAlways},
		// All clauses + negative / float / NO CYCLE.
		{"CREATE TABLE T (id INT64 GENERATED ALWAYS AS IDENTITY (START WITH -5 INCREMENT BY 2 MAXVALUE 100 MINVALUE -100 NO CYCLE)) PRIMARY KEY (id)", ast.GenModeGeneratedAlways},
		// signed_numeric_literal: a typed numeric_literal whose string_literal is two
		// ADJACENT components (GoogleSQL adjacent-string concatenation).
		{"CREATE TABLE T (id INT64 GENERATED ALWAYS AS IDENTITY (START WITH NUMERIC '1' '2')) PRIMARY KEY (id)", ast.GenModeGeneratedAlways},
	}
...
func TestCreateTable_GeneratedIdentity_Rejects(t *testing.T) {
