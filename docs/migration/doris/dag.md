...
| **T1.3** | expressions (operators, function calls, CASE, CAST, subqueries, IN/BETWEEN/LIKE/REGEXP, window functions, hints) | `doris/parser` | T1.2 | T1.8 | 1 | P0 | **done** (PR #59) |
| **T1.4** | SELECT core (list, FROM, WHERE, GROUP BY, HAVING, QUALIFY, ORDER BY, LIMIT/OFFSET) | `doris/parser` | T1.3 | T1.8 | 1 | P0 | **done** (PR #61) |
| **T1.5** | joins (INNER/LEFT/RIGHT/FULL OUTER/CROSS with ON/USING, execution hints) | `doris/parser` | T1.4 | T1.6, T1.7, T1.8 | 1 | P0 | **done** (PR #66) |
| **T1.6** | CTEs (WITH common table expressions) | `doris/parser` | T1.4 | T1.5, T1.7, T1.8 | 1 | P0 | **done** (PR #67) |
| **T1.7** | set operators (UNION [ALL], EXCEPT/MINUS, INTERSECT) | `doris/parser` | T1.4 | T1.5, T1.6, T1.8 | 1 | P0 | **done** (PR #68) |
| **T1.8** | statement-classification helper (DDL/DML/SELECT/SelectInfoSchema) | `doris/analysis` | F4 | T1.1--T1.7 | 1 | P0 | **done** (PR #55) |
| **T2.1** | DDL: CREATE TABLE (full -- column declarations, constraints, AGGREGATE/UNIQUE/DUPLICATE KEY, DISTRIBUTED BY, PARTITION BY range/list, ROLLUP, ENGINE, properties) | `doris/parser` | T1.2, T1.3 | T2.3, T2.4, T2.5 | 2 | P0 | **done** (PR #62) |
| **T2.2** | DDL: ALTER TABLE (columns, partitions, properties, rollups, distribution, engine, comment) | `doris/parser` | T2.1 | T2.3, T2.4, T2.5 | 2 | P0 | **done** (PR #69) |
| **T2.3** | DDL: DATABASE / SCHEMA (CREATE/ALTER/DROP incl. FORCE) | `doris/parser` | T1.1 | T2.1, T2.2, T2.4, T2.5 | 2 | P0 | **done** (PR #58) |
| **T2.4** | DDL: VIEW (CREATE OR REPLACE / ALTER / DROP) | `doris/parser` | T1.4 | T2.1, T2.2, T2.3, T2.5 | 2 | P0 | **done** (PR #70) |
...
| **T3.1** | query span extractor (result-column lineage, table-access, CTE tracking, set-op merging, subquery field resolution) | `doris/analysis` | T1.4, T1.5, T1.6, T1.7 | T3.2 | 3 | P0 | **done** (PR #77) |
| **T3.2** | syntax diagnostics (error listing for editor) | `doris/parser` | F4 | T3.1 | 3 | P0 | **done** (PR #56) |
| **T4.1** | DML: INSERT (INTO/OVERWRITE, partitions, labels, column lists, hints) | `doris/parser` | T1.4 | T4.2, T4.3 | 4 | P0 | **done** (PR #78) |
| **T4.2** | DML: UPDATE / DELETE (SET, FROM, partitions, USING, WHERE) | `doris/parser` | T1.4, T1.5 | T4.1, T4.3 | 4 | P0 | **done** (PR #79) |
| **T4.3** | DML: MERGE INTO (WHEN MATCHED / NOT MATCHED, conditional predicates) | `doris/parser` | T1.4, T1.5 | T4.1, T4.2 | 4 | P0 | **done** (PR #80) |
...
| **T5.1** | DDL: Materialized Views MTMV (CREATE with build mode/refresh triggers/partition/distribution, ALTER, DROP, REFRESH, PAUSE, RESUME, CANCEL) | `doris/parser` | T2.1, T1.4 | T5.2--T5.5 | 5 | P1 | **done** (PR #85) |
| **T5.2** | DDL: Catalog (CREATE/ALTER/DROP for external sources -- HIVE, ICEBERG, Delta Lake, Paimon) | `doris/parser` | T1.1 | T5.1, T5.3--T5.5 | 5 | P1 | **done** (PR #86) |
| **T5.3** | DDL: Storage infrastructure (STORAGE VAULT, STORAGE POLICY, REPOSITORY, STAGE, FILE) | `doris/parser` | T1.1 | T5.1, T5.2, T5.4, T5.5 | 5 | P1 | **done** (PR #88) |
| **T5.4** | DDL: Workload management (WORKLOAD GROUP/POLICY, RESOURCE, COMPUTE GROUP, SQL BLOCK RULE) | `doris/parser` | T1.1 | T5.1--T5.3, T5.5 | 5 | P1 | **done** (PR #89) |
| **T5.5** | DDL: Security (ROLE, USER, ROW POLICY, ENCRYPTION KEY, DICTIONARY) | `doris/parser` | T1.1 | T5.1--T5.4 | 5 | P1 | **done** (PR #90) |
| **T6.1** | DML: TRUNCATE TABLE, COPY INTO, LOAD (bulk), EXPORT | `doris/parser` | T1.4 | T6.2 | 6 | P1 | **done** (PR #144) |
| **T6.2** | DML: ROUTINE LOAD (CREATE/PAUSE/RESUME/STOP/SHOW), SYNC | `doris/parser` | T1.1 | T6.1 | 6 | P1 | **done** (PR #145) |
| **T7.1** | DCL: GRANT / REVOKE (table, resource, role privileges) | `doris/parser` | T1.1 | T7.2--T7.5 | 7 | P1 | **done** (PR #146) |
| **T7.2** | TCL: BEGIN WITH LABEL, COMMIT, ROLLBACK (WORK/CHAIN/RELEASE) | `doris/parser` | F4 | T7.1, T7.3--T7.5 | 7 | P1 | **done** (PR #147) |
| **T7.3** | Utility: SHOW (40+ variants), DESCRIBE / EXPLAIN, USE (database/catalog/cloud cluster), SET / UNSET, HELP | `doris/parser` | T1.1, T1.3 | T7.1, T7.2, T7.4, T7.5 | 7 | P1 | **done** (PR #148) |
| **T7.4** | Admin: ADMIN commands (replica, tablet, backend diagnostics), SYSTEM ALTER (backends, frontends, brokers) | `doris/parser` | T1.1 | T7.1--T7.3, T7.5 | 7 | P1 | **done** (PR #149) |
| **T7.5** | Utility: BACKUP / RESTORE, KILL, LOCK/UNLOCK, PLUGIN, WARM UP, REFRESH, CLEAN, CANCEL, RECOVER | `doris/parser` | T1.1 | T7.1--T7.4 | 7 | P1 | **done** (PR #150) |
