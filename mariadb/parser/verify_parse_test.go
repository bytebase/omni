...
//
// Corpus files live in mariadb/quality/corpus/*.sql with annotations:
//
...
// Run:
//
//	go test ./mariadb/parser/ -run TestVerifyCorpus -count=1 -v
func TestVerifyCorpus(t *testing.T) {
	corpusDir := filepath.Join("..", "quality", "corpus")
	entries, err := os.ReadDir(corpusDir)
	if err != nil {
		corpusDir = filepath.Join("mariadb", "quality", "corpus")
		entries, err = os.ReadDir(corpusDir)
		if err != nil {
	...
	if len(corpusFiles) == 0 {
		t.Skip("No corpus files found in mariadb/quality/corpus/")
	}
