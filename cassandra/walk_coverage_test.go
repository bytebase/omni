...
func TestWalkChildrenCoverage(t *testing.T) {
	...
	for _, sql := range []string{
		...
		"ALTER TABLE t RENAME IF EXISTS a TO b AND c TO d",
		...
		"SELECT CAST(col AS int) FROM t",
		"INSERT INTO t (id) VALUES (?)",
		"SELECT * FROM t WHERE id = :myid",
		"INSERT INTO t (id) VALUES (NaN)",
		"INSERT INTO t (id) VALUES (Infinity)",
		"INSERT INTO t (id) VALUES (-Infinity)",
		"DROP FUNCTION IF EXISTS ks.f(int, text)",
		"DROP AGGREGATE IF EXISTS ks.agg(int)",
		"UPDATE t SET x = 1 WHERE id = 1 IF x IN (1, 2, 3)",
		"UPDATE t SET x = 1 WHERE id = 1 IF tags CONTAINS 'admin'",
		"UPDATE t SET x = 1 WHERE id = 1 IF tags CONTAINS KEY 'role'",
		"INSERT INTO t JSON '{\"id\": 1}' DEFAULT NULL",
		"CREATE ROLE r WITH HASHED PASSWORD = 'hash'",
		"CREATE ROLE r WITH ACCESS TO DATACENTERS {'dc1', 'dc2'}",
		"CREATE ROLE r WITH ACCESS TO ALL DATACENTERS",
		"GRANT SELECT ON MBEAN 'org.example:type=Foo' TO r",
		"GRANT SELECT ON ALL MBEANS TO r",
		"GRANT EXECUTE ON FUNCTION ks.f(int, text) TO r",
		"UPDATE t SET addr.street = 'Main St' WHERE id = 1",
		"DELETE addr.street FROM t WHERE id = 1",
	} {
...
func collectAllNodeStructTypes() []string {
	...
	candidates := []reflect.Type{
		...
		reflect.TypeOf(ast.AlterTableRenameItem{}),
