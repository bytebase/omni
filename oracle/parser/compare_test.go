...
// TestParseAlterSessionSystem tests ALTER SESSION and ALTER SYSTEM statements (batch 98).
func TestParseAlterSessionSystem(t *testing.T) {
	tests := []string{
		...
		"ALTER SESSION CLOSE DATABASE LINK my_link",
		...
		"ALTER SESSION ENABLE RESUMABLE TIMEOUT 3600 NAME 'my_batch_job'",
		...
		"ALTER SYSTEM SET open_cursors = 300 COMMENT = 'increase cursors' DEFERRED SCOPE = SPFILE SID = '*'",
		...
		"ALTER SYSTEM KILL SESSION '12,34' IMMEDIATE",
		"ALTER SYSTEM KILL SESSION '12,34', @2 FORCE NOREPLAY TIMEOUT 60",
		// ALTER SYSTEM - DISCONNECT SESSION
		"ALTER SYSTEM DISCONNECT SESSION '12,34' POST_TRANSACTION",
		"ALTER SYSTEM DISCONNECT SESSION '12,34' IMMEDIATE",
		// ALTER SYSTEM - FLUSH
		...
		"ALTER SYSTEM FLUSH REDO TO standby_db NO CONFIRM APPLY",
		...
		"ALTER SYSTEM ARCHIVE LOG INSTANCE 'inst1' GROUP 5 THREAD 2 TO '/archive/'",
		"ALTER SYSTEM ARCHIVE LOG LOGFILE '/redo/redo01.log' USING BACKUP CONTROLFILE",
		"ALTER SYSTEM ARCHIVE LOG CHANGE 12345",
		...
		"ALTER SYSTEM CANCEL SQL '12,34'",
		"ALTER SYSTEM CANCEL SQL '12,34', @2 SQL_ID 'abc123def'",
		// ALTER SYSTEM - SHUTDOWN dispatcher
		"ALTER SYSTEM SHUTDOWN 'dispatcher1'",
		"ALTER SYSTEM SHUTDOWN IMMEDIATE 'dispatcher1'",
		// ALTER SYSTEM - RELOCATE CLIENT
		"ALTER SYSTEM RELOCATE CLIENT 'client123'",
		// ALTER SYSTEM - ROLLING MIGRATION/PATCH
		...
		"ALTER SYSTEM SET ENCRYPTION WALLET OPEN IDENTIFIED BY my_password",
...
// TestParseBatch101_PLSQLObjects tests batch 101: PL/SQL objects (review+fix).
// Covers CREATE/ALTER/DROP for PROCEDURE, FUNCTION, PACKAGE, TRIGGER, TYPE.
func TestParseBatch101_PLSQLObjects(t *testing.T) {
	tests := []string{
		// CREATE PROCEDURE - IF NOT EXISTS
		"CREATE IF NOT EXISTS PROCEDURE my_proc IS BEGIN NULL; END;",
		// CREATE PROCEDURE - EDITIONABLE with SHARING
		"CREATE OR REPLACE EDITIONABLE PROCEDURE hr.my_proc SHARING = METADATA IS BEGIN NULL; END;",
		// CREATE FUNCTION - IF NOT EXISTS (comes after FUNCTION per BNF)
		"CREATE FUNCTION IF NOT EXISTS get_total (p_id IN NUMBER) RETURN NUMBER IS BEGIN RETURN 0; END;",
		// CREATE FUNCTION - EDITIONABLE with SHARING
		"CREATE OR REPLACE EDITIONABLE FUNCTION calc RETURN NUMBER SHARING = METADATA DETERMINISTIC IS BEGIN RETURN 1; END;",
		// CREATE FUNCTION - AGGREGATE USING
		"CREATE FUNCTION agg_fn RETURN NUMBER AGGREGATE USING agg_impl IS BEGIN RETURN 0; END;",
		// CREATE PACKAGE - IF NOT EXISTS with SHARING
		"CREATE IF NOT EXISTS PACKAGE my_pkg SHARING = METADATA IS\n  PROCEDURE do_it;\nEND my_pkg;",
		// CREATE PACKAGE - EDITIONABLE with AUTHID
		"CREATE OR REPLACE EDITIONABLE PACKAGE my_pkg AUTHID CURRENT_USER IS\n  FUNCTION get_val RETURN NUMBER;\nEND my_pkg;",
		// CREATE PACKAGE BODY - IF NOT EXISTS
		"CREATE IF NOT EXISTS PACKAGE BODY my_pkg IS\n  PROCEDURE do_it IS\n  BEGIN\n    NULL;\n  END;\nEND my_pkg;",
		// CREATE TRIGGER - IF NOT EXISTS
		"CREATE IF NOT EXISTS TRIGGER my_trigger BEFORE INSERT ON employees BEGIN NULL; END;",
		// CREATE TRIGGER - EDITIONABLE
		"CREATE OR REPLACE EDITIONABLE TRIGGER audit_trigger AFTER UPDATE ON employees FOR EACH ROW BEGIN NULL; END;",
		// CREATE TYPE - IF NOT EXISTS
		"CREATE IF NOT EXISTS TYPE my_type AS OBJECT (id NUMBER, name VARCHAR2(100))",
		// CREATE TYPE BODY - NONEDITIONABLE
		"CREATE OR REPLACE NONEDITIONABLE TYPE BODY my_type IS\n  MEMBER FUNCTION get_id RETURN NUMBER IS\n  BEGIN\n    RETURN SELF.id;\n  END;\nEND;",
		// ALTER PACKAGE - IF EXISTS
		"ALTER PACKAGE IF EXISTS my_pkg COMPILE",
		// ALTER PACKAGE - COMPILE DEBUG BODY (BNF order: DEBUG before target)
		"ALTER PACKAGE my_pkg COMPILE DEBUG BODY",
		// ALTER PACKAGE - COMPILE with EDITIONABLE
		"ALTER PACKAGE my_pkg COMPILE BODY EDITIONABLE",
		// ALTER TYPE - REPLACE clause
		"ALTER TYPE my_type REPLACE AS OBJECT (id NUMBER, name VARCHAR2(200), email VARCHAR2(100))",
		// ALTER TYPE - MODIFY modifier_clause (MODIFY NOT FINAL)
		"ALTER TYPE my_type MODIFY NOT FINAL CASCADE",
		// DROP TYPE - VALIDATE
		"DROP TYPE my_type VALIDATE",
		// DROP TYPE - FORCE
		"DROP TYPE IF EXISTS my_schema.my_type FORCE",
		// DROP FUNCTION - IF EXISTS
		"DROP FUNCTION IF EXISTS hr.my_func",
		// DROP PROCEDURE - IF EXISTS
		"DROP PROCEDURE IF EXISTS my_proc",
		// DROP PACKAGE BODY - IF EXISTS
		"DROP PACKAGE BODY IF EXISTS my_pkg",
...
// TestAlterSystem tests ALTER SYSTEM statement parsing (batch 111).
// Covers all BNF branches including edge cases for FLUSH, CHECKPOINT,
// ARCHIVE LOG, CANCEL SQL, DISCONNECT/KILL SESSION, ROLLING, encryption, etc.
func TestAlterSystem(t *testing.T) {
	tests := []string{
		...
		"ALTER SYSTEM FLUSH FLASH_CACHE GLOBAL",
		"ALTER SYSTEM FLUSH FLASH_CACHE LOCAL",
		"ALTER SYSTEM FLUSH REDO TO standby_db",
		"ALTER SYSTEM FLUSH REDO TO standby_db CONFIRM APPLY",
		"ALTER SYSTEM FLUSH REDO TO standby_db NO CONFIRM APPLY",
		"ALTER SYSTEM FLUSH PASSWORDFILE_METADATA_CACHE",
		// CHECKPOINT
		"ALTER SYSTEM CHECKPOINT",
		...
		// SET with multiple params and modifiers
		"ALTER SYSTEM SET open_cursors = 300 SCOPE = MEMORY",
		"ALTER SYSTEM SET db_block_size = 8192 COMMENT = 'set block size' DEFERRED SCOPE = SPFILE SID = '*' CONTAINER = ALL",
		// RESET
		"ALTER SYSTEM RESET open_cursors SCOPE = BOTH SID = 'inst1'",
		// KILL SESSION with all options
		"ALTER SYSTEM KILL SESSION '123,456'",
		"ALTER SYSTEM KILL SESSION '123,456', @3 IMMEDIATE",
		"ALTER SYSTEM KILL SESSION '123,456' FORCE NOREPLAY TIMEOUT 120",
		// DISCONNECT SESSION
		"ALTER SYSTEM DISCONNECT SESSION '123,456' POST_TRANSACTION IMMEDIATE",
		// ARCHIVE LOG variants
		"ALTER SYSTEM ARCHIVE LOG SEQUENCE 100",
		"ALTER SYSTEM ARCHIVE LOG CHANGE 999",
		"ALTER SYSTEM ARCHIVE LOG CURRENT",
		"ALTER SYSTEM ARCHIVE LOG CURRENT NOSWITCH",
		"ALTER SYSTEM ARCHIVE LOG GROUP 3",
		"ALTER SYSTEM ARCHIVE LOG LOGFILE '/redo/redo01.log'",
		"ALTER SYSTEM ARCHIVE LOG LOGFILE '/redo/redo01.log' USING BACKUP CONTROLFILE",
		"ALTER SYSTEM ARCHIVE LOG NEXT",
		"ALTER SYSTEM ARCHIVE LOG ALL",
		"ALTER SYSTEM ARCHIVE LOG INSTANCE 'inst1' ALL THREAD 2 TO '/archive/'",
		// SUSPEND / RESUME
		"ALTER SYSTEM SUSPEND",
		"ALTER SYSTEM RESUME",
		// QUIESCE
		...
		// CANCEL SQL
		"ALTER SYSTEM CANCEL SQL '12,34'",
		"ALTER SYSTEM CANCEL SQL '12,34', @5 SQL_ID 'g3q7v5zn1h2k8'",
		// SHUTDOWN dispatcher
		"ALTER SYSTEM SHUTDOWN 'my_dispatcher'",
		"ALTER SYSTEM SHUTDOWN IMMEDIATE 'my_dispatcher'",
		// RELOCATE CLIENT
		"ALTER SYSTEM RELOCATE CLIENT 'client_abc'",
		// ROLLING MIGRATION / PATCH
		"ALTER SYSTEM START ROLLING MIGRATION TO '19.0.0'",
		"ALTER SYSTEM STOP ROLLING MIGRATION",
		"ALTER SYSTEM START ROLLING PATCH",
		"ALTER SYSTEM STOP ROLLING PATCH",
		// SET ENCRYPTION
		"ALTER SYSTEM SET ENCRYPTION WALLET OPEN IDENTIFIED BY secret_pw",
		"ALTER SYSTEM SET ENCRYPTION WALLET CLOSE",
		"ALTER SYSTEM SET ENCRYPTION WALLET CLOSE IDENTIFIED BY secret_pw",
		"ALTER SYSTEM SET ENCRYPTION KEY",
		"ALTER SYSTEM SET ENCRYPTION KEY IDENTIFIED BY secret_pw",
		// SWITCH LOGFILE
		"ALTER SYSTEM SWITCH LOGFILE",
	}
	for _, sql := range tests {
