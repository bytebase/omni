...
// TestBlockCommentDoesNotNest_Oracle proves against a live MySQL 8.0 container
// that omni's parser now agrees with the server: a "/* /* */ ... */"-shaped
// comment closes at the FIRST */, so text after it is a live, separate
// statement — not comment content the way the old depth-nesting scanner read
// it. The oracle runs the raw SQL on a multiStatements connection so the
...
// TestBlockCommentDoesNotHidePredicate_Oracle proves against a live MySQL 8.0
// container that a predicate following a "/* /* */ ... */"-shaped comment is
// live and takes effect, matching omni's parsed WHERE clause. The old
