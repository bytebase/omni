...
func (p *Parser) parseRedshiftUseStmt(loc int) (nodes.Node, error) {
	database, err := p.parseColId()
	if err != nil {
		return nil, err
	}
	return &nodes.VariableSetStmt{
		...
		Name: "database",
		Args: &nodes.List{Items: []nodes.Node{makeStringConst(database)}},
...
func (p *Parser) parseVariableShowStmt() (nodes.Node, error) {
	...
	if p.isRedshiftShowStart() {
		return p.parseRedshiftShowStmt(loc)
	}
	switch p.cur.Type {
	case TIME:
		p.advance() // consume TIME
	...
	case CURRENT_USER:
		p.advance()
		return &nodes.VariableShowStmt{Name: "current_user", Loc: nodes.Loc{Start: loc, End: p.prev.End}}, nil

	case SESSION_USER:
		p.advance()
		return &nodes.VariableShowStmt{Name: "session_user", Loc: nodes.Loc{Start: loc, End: p.prev.End}}, nil
