...
func (p *Parser) parseDropStmtInner() (nodes.Node, error) {
	if objectType := p.redshiftDropObjectType(); objectType != "" {
		return p.parseRedshiftObjectStmt("drop", objectType, p.stmtStart)
	}
	switch p.cur.Type {
...
func (p *Parser) parseDropNameList(objType nodes.ObjectType) (nodes.Node, error) {
	...
	options, err := p.parseRedshiftDropSchemaOptions(objType)
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseRedshiftDropSchemaOptions(objType nodes.ObjectType) (*nodes.List, error) {
	if objType != nodes.OBJECT_SCHEMA || p.cur.Type != DROP {
		return nil, nil
	}
	p.advance() // consume DROP
	if _, err := p.expect(EXTERNAL); err != nil {
		return nil, err
	}
	...
	return &nodes.List{Items: []nodes.Node{
		makeDefElem("drop_external_database", &nodes.Boolean{Boolval: true}),
	}}, nil
}
