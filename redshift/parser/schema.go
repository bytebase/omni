...
// parseRedshiftSchemaQuota parses Redshift's optional schema storage quota:
// QUOTA { quota [MB | GB | TB] | UNLIMITED }.
func (p *Parser) parseRedshiftSchemaQuota() (*nodes.List, error) {
	if !p.redshiftWordEqual("quota") {
		return nil, nil
	}
	p.advance()

	quota, err := p.parseRedshiftSchemaQuotaValue()
	if err != nil {
		return nil, err
	}
	return &nodes.List{Items: []nodes.Node{makeDefElem("quota", &nodes.String{Str: quota})}}, nil
}

func (p *Parser) parseRedshiftSchemaQuotaValue() (string, error) {
	if p.redshiftWordEqual("unlimited") {
		value := strings.ToUpper(p.advance().Str)
		return value, nil
	}
