...
func (p *Parser) parseSimpleSelectCore() (*nodes.SelectStmt, error) {
	...
	if err := p.parseRedshiftTopClause(stmt); err != nil {
		return nil, err
	}
	...
	if p.collectMode() {
		...
		for _, t := range []int{
			EXCLUDE, INTO, FROM, WHERE, GROUP_P, HAVING, QUALIFY, WINDOW,
	...
	if p.cur.Type == EXCLUDE {
		stmt.ExcludeList, err = p.parseRedshiftExcludeList()
		if err != nil {
			return nil, err
		}
	}
	...
	if p.cur.Type == FROM {
		...
		if p.collectMode() {
			for _, t := range []int{
				WHERE, GROUP_P, HAVING, QUALIFY, WINDOW,
				ORDER, LIMIT, OFFSET, FETCH, FOR,
				UNION, EXCEPT, INTERSECT, ';',
			} {
				p.addTokenCandidate(t)
	...
	if p.cur.Type == GROUP_P {
		...
		if p.collectMode() {
			for _, t := range []int{
				HAVING, QUALIFY, WINDOW, ORDER, LIMIT, OFFSET, FETCH, FOR,
				UNION, EXCEPT, INTERSECT, ';',
			} {
				p.addTokenCandidate(t)
			}
	...
	if p.cur.Type == QUALIFY {
		p.advance()
		stmt.QualifyClause, err = p.parseAExpr(0)
		if err != nil {
			return nil, err
		}
		if !p.collectMode() && stmt.QualifyClause == nil {
			return nil, p.syntaxErrorAtCur()
		}
	}
...
func (p *Parser) parseRedshiftTopClause(stmt *nodes.SelectStmt) error {
	if stmt.LimitCount != nil || !p.curIdentEquals("top") {
		return nil
	}
	...
	stmt.LimitCount = &nodes.A_Const{
		Val: &nodes.Integer{Ival: p.cur.Ival},
		Loc: nodes.Loc{Start: p.cur.Loc, End: p.cur.End},
	}
...
func (p *Parser) parseRedshiftExcludeList() (*nodes.List, error) {
...
func (p *Parser) curIdentEquals(word string) bool {
	return p.cur.Type == IDENT && strings.EqualFold(p.cur.Str, word)
}
