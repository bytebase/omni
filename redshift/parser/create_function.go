...
func (p *Parser) parseCreateFunctionStmt(startLoc int, isReplace bool) (*nodes.CreateFunctionStmt, error) {
	...
	if isProcedure {
		...
		var opts *nodes.List
		if p.consumeRedshiftWord("nonatomic") {
			opts = appendToList(opts, makeDefElem("nonatomic", &nodes.Boolean{Boolval: true}))
		}
		var err error
		parsedOpts, err := p.parseOptCreatefuncOptList()
		if err != nil {
			return nil, err
		}
		opts = concatNodeLists(opts, parsedOpts)
		stmt.Options = opts
