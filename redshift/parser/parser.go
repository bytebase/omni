...
func (p *Parser) parseStmt() (nodes.Node, error) {
	...
	case ALTER:
		...
		if objectType := p.redshiftAlterObjectType(); objectType != "" {
			return p.parseRedshiftObjectStmt("alter", objectType, alterLoc)
		}
		switch p.cur.Type {
		case DATABASE:
	...
	case DESC:
		...
		return p.parseRedshiftDescStmt(loc)
	case RESET:
	...
	default:
		if p.cur.Type == ATTACH {
			loc := p.pos()
			p.advance()
			return p.parseRedshiftAttachDetachStmt("attach", loc)
		}
		if p.cur.Type == DETACH {
			loc := p.pos()
			p.advance()
			return p.parseRedshiftAttachDetachStmt("detach", loc)
		}
		if p.redshiftWordEqual("unload") {
			loc := p.pos()
			p.advance()
			return p.parseUnloadStmt(loc)
		}
		if p.redshiftWordEqual("describe") {
			loc := p.pos()
			p.advance()
			return p.parseRedshiftDescStmt(loc)
		}
		if p.redshiftWordEqual("use") {
			loc := p.pos()
			p.advance()
			return p.parseRedshiftUseStmt(loc)
		}
		if p.redshiftWordEqual("cancel") {
			loc := p.pos()
			p.advance()
			return p.parseRedshiftCancelStmt(loc)
		}
		return nil, nil
	}
}
...
func (p *Parser) parseCreateDispatch() (nodes.Node, error) {
	...
	next := p.peekNext()
	if isRedshiftCreateObjectLead(next) {
		p.advance() // consume CREATE
		objectType := p.redshiftCreateObjectType()
		if objectType != "" {
			return p.parseRedshiftObjectStmt("create", objectType, createLoc)
		}
	}
	switch next.Type {
	case OR:
		...
		p.expect(REPLACE)
		if objectType := p.redshiftCreateObjectType(); objectType != "" {
			return p.parseRedshiftObjectStmt("create or replace", objectType, createLoc)
		}
		switch p.cur.Type {
...
func (p *Parser) parseCreateTempDispatch() (nodes.Node, error) {
	...
	names, hashTemp, err := p.parseRedshiftCreateTableName()
	if err != nil {
		return nil, err
	}
	if hashTemp {
		relpersistence = byte(nodes.RELPERSISTENCE_TEMP)
	}
	...
	if p.cur.Type == AS || p.cur.Type == USING || p.isRedshiftCreateTableAttributeStart() {
...
func (p *Parser) parseRedshiftCreateTableName() (*nodes.List, bool, error) {
	if p.cur.Type == Op && p.cur.Str == "#" {
		p.advance()
		name, err := p.parseName()
		if err != nil {
			return nil, false, err
		}
		return &nodes.List{Items: []nodes.Node{&nodes.String{Str: name}}}, true, nil
	}
...
func (p *Parser) parseCreateTableOrCTASAfterParen(names *nodes.List, relpersistence byte, ifNotExists bool) (nodes.Node, error) {
	...
	if p.cur.Type == ')' {
		...
		stmt.Options, err = p.parseRedshiftCreateTableAttributesInto(stmt.Options)
		if err != nil {
			return nil, err
		}
		return stmt, nil
...
func (p *Parser) finishCTAS(names *nodes.List, colNames *nodes.List, relpersistence byte, ifNotExists bool) (*nodes.CreateTableAsStmt, error) {
	...
	options, err := p.parseRedshiftCreateTableAttributesInto(options)
	if err != nil {
		return nil, err
	}
