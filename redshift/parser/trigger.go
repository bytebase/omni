...
func (p *Parser) parseCreateTrigStmt(replace bool) (*nodes.CreateTrigStmt, error) {
	...
	if p.cur.Type == FUNCTION || p.cur.Type == PROCEDURE {
		p.advance()
	}
...
func (p *Parser) parseTriggerActionTime() int64 {
	...
	case BEFORE:
		p.advance()
		return int64(nodes.TRIGGER_TYPE_BEFORE)
	case AFTER:
		p.advance()
	...
	case INSTEAD:
		p.advance()
		p.expect(OF)
		return int64(nodes.TRIGGER_TYPE_INSTEAD)
	}
	return int64(nodes.TRIGGER_TYPE_AFTER)
}
...
func (p *Parser) parseTriggerOneEvent() (int64, *nodes.List) {
	...
	case INSERT:
		p.advance()
		return int64(nodes.TRIGGER_TYPE_INSERT), nil
	case DELETE_P:
		p.advance()
		return int64(nodes.TRIGGER_TYPE_DELETE), nil
	case UPDATE:
		p.advance()
		if p.cur.Type == OF {
			p.advance()
			return int64(nodes.TRIGGER_TYPE_UPDATE), p.parseColumnList()
		}
		return int64(nodes.TRIGGER_TYPE_UPDATE), nil
	case TRUNCATE:
		p.advance()
		return int64(nodes.TRIGGER_TYPE_TRUNCATE), nil
	}
	return 0, nil
}
...
func (p *Parser) parseTriggerTransition() (*nodes.TriggerTransition, error) {
	...
	case OLD:
		isNew = false
		p.advance()
	// optional-probe: trigger transition list terminator — referencing loop breaks on nil.
	default:
		return nil, nil
	}
	isTable := false
...
func (p *Parser) parseTriggerFuncArg() (nodes.Node, error) {
	...
	case ICONST:
		tok := p.advance()
		return &nodes.String{Str: strconv.FormatInt(tok.Ival, 10)}, nil
	case FCONST:
		tok := p.advance()
		return &nodes.String{Str: tok.Str}, nil
	case SCONST:
		tok := p.advance()
		return &nodes.String{Str: tok.Str}, nil
	default:
		label, _ := p.parseColLabel()
		return &nodes.String{Str: label}, nil
	}
}
