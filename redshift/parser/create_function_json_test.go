...
func TestRedshiftCreateProcedureNonAtomicParse(t *testing.T) {
	stmts, err := Parse(`CREATE OR REPLACE PROCEDURE test_transaction_sp(batch_size int) NONATOMIC AS $$
BEGIN
    COMMIT;
END;
$$ LANGUAGE plpgsql;`)
	...
	cf := firstCreateFunction(t, stmts)
	assertDefElemBool(t, cf.Options, "nonatomic", true)
}
