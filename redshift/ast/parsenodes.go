...
type SelectStmt struct {
	...
	QualifyClause  Node        // Redshift QUALIFY conditional-expression
	WindowClause   *List       // WINDOW window_name AS (...), ...
	ExcludeList    *List       // Redshift SELECT * EXCLUDE (...) result names
...
type DropStmt struct {
	...
	Options    *List // Redshift options (list of DefElem)
	Loc        Loc   // token location
}
...
type MergeStmt struct {
	...
	RemoveDuplicates bool        // Redshift REMOVE DUPLICATES shortcut
	ReturningList    *List       // list of expressions to return
	WithClause       *WithClause // WITH clause
...
const (
	...
	ACL_TARGET_SCOPED                               // Redshift scoped permissions for future objects
)
...
// UnloadStmt represents a Redshift UNLOAD statement.
type UnloadStmt struct {
	Query   string // SELECT query text inside UNLOAD (...)
	Target  string // S3 target URI
	Options *List  // list of DefElem
	Loc     Loc    // token location
}

func (n *UnloadStmt) Tag() NodeTag { return T_UnloadStmt }

// RedshiftShowStmt represents a Redshift SHOW utility statement.
type RedshiftShowStmt struct {
	Command string // DATABASES, SCHEMAS, TABLES, COLUMNS, etc.
	Options *List  // raw option tokens as DefElem/String nodes
	Loc     Loc    // token location
}

func (n *RedshiftShowStmt) Tag() NodeTag { return T_RedshiftShowStmt }

// RedshiftDescStmt represents a Redshift DESC utility statement.
type RedshiftDescStmt struct {
	ObjectType string // datashare, identity provider, etc.
	Name       *List  // object name
	Options    *List  // raw option tokens as DefElem/String nodes
	Loc        Loc    // token location
}

func (n *RedshiftDescStmt) Tag() NodeTag { return T_RedshiftDescStmt }

// RedshiftObjectStmt represents Redshift-specific object DDL that is not part
// of PostgreSQL grammar.
type RedshiftObjectStmt struct {
	Command    string // create, alter, drop, attach, detach
	ObjectType string // datashare, external schema, masking policy, etc.
	Name       *List  // primary object name, when present
	Options    *List  // raw option tokens as String nodes
	Loc        Loc    // token location
}

func (n *RedshiftObjectStmt) Tag() NodeTag { return T_RedshiftObjectStmt }
...
type RefreshMatViewStmt struct {
	...
	Behavior   int       // RESTRICT or CASCADE behavior
	Loc        Loc       // token location
}
