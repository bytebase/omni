...
func TestRoutineGrammarRejected(t *testing.T) {
	...
	cases := []string{
		...
		`SHOW FUNCTION`,
		`SHOW PROCEDURE`,
		// TiDB v8.5.0 CREATE PROCEDURE has no characteristic clause; MySQL's
		// DETERMINISTIC / {NO|CONTAINS|READS|MODIFIES} SQL / SQL SECURITY /
		// LANGUAGE / COMMENT are all rejected (container-verified, BYT-9650).
		`CREATE PROCEDURE p() DETERMINISTIC BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() NOT DETERMINISTIC BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() NO SQL BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() CONTAINS SQL BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() READS SQL DATA BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() MODIFIES SQL DATA BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() SQL SECURITY DEFINER BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() SQL SECURITY INVOKER BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() LANGUAGE SQL BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() COMMENT 'x' BEGIN SELECT 1; END`,
		`CREATE PROCEDURE p() COMMENT 'c' LANGUAGE SQL DETERMINISTIC BEGIN SELECT 1; END`,
	}
	for _, sql := range cases {
		t.Run(sql, func(t *testing.T) {
			if _, err := Parse(sql); err == nil {
